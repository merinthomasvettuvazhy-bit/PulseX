...
function processCommand(command) {
  ...
  if (cmd.includes("call women helpline") || cmd.includes("women")) {
    speakText("Calling women helpline");
    setTimeout(() => window.location.href = "tel:1091", 500);
    return;
  }

  if (cmd.includes("call ambulance") || cmd.includes("ambulance")) {
    speakText("Calling ambulance");
    setTimeout(() => window.location.href = "tel:108", 500);
    return;
  }

  if (cmd.includes("call child helpline") || cmd.includes("child")) {
    speakText("Calling child helpline");
    setTimeout(() => window.location.href = "tel:1098", 500);
    return;
  }

  if (
  cmd.includes("call") &&
  cmd.includes(label.toLowerCase())
) {
